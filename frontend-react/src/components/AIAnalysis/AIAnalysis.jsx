...
function ResumePanel() {
    ...
    async function analyze() {
        if (!file) return toast.error('Please upload your resume file (PDF or Image).');
        setLoading(true);
        try {
            ...
            setResult(await api.analyzeResume(formData)); 
        }
    ...
    return (
        <div>
            ...
            <div className="form-group" style={{ marginBottom: '1rem' }}>
                <label className="form-label">Upload Resume (PDF, PNG, JPG)</label>
                <input className="form-input" type="file" accept=".pdf,image/png,image/jpeg,image/jpg" onChange={e => setFile(e.target.files[0])} style={{ padding: '0.6rem' }} />
            </div>
...
function JobMatchPanel({ students, lockedId }) {
    ...
    async function match() {
        if (!sid && !file) return toast.error('Select a student or upload a resume.');
        setLoading(true);
        try { 
            if (file) {
                const formData = new FormData();
                formData.append('resume_file', file);
                setResult(await api.matchJobsWithResume(formData));
            } else {
                setResult(await api.getJobRecommendations(sid));
            }
        }
        catch (e) { toast.error(e.message || 'Failed to match jobs.'); }
        finally { setLoading(false); }
    }
    ...
    return (
        <div>
            ...
            <div className="form-group" style={{ marginBottom: '1rem', marginTop: '1rem' }}>
                <label className="form-label">Or Upload Resume directly (PDF, Image)</label>
                <input className="form-input" type="file" accept=".pdf,image/*" onChange={e => setFile(e.target.files[0])} style={{ padding: '0.6rem' }} />
                {file && <p style={{ fontSize: '0.8rem', color: 'var(--accent-emerald)', marginTop: '0.5rem' }}>Using uploaded resume for matching.</p>}
            </div>

            <button className="btn btn-primary" onClick={match} disabled={loading || (!sid && !file)}>
