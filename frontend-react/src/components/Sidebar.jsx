...
const NAV = [
    ...
    { id: 'ai-interview',     label: 'AI Interview',     icon: 'fas fa-microphone-alt',  roles: ['student'], badge: 'NEW' },
    { id: 'virtual-campus',   label: 'Virtual Campus',   icon: 'fas fa-building',        roles: ['admin', 'student'], badge: '3D' },
    { id: 'cohort-analytics', label: 'Cohort Analytics', icon: 'fas fa-chart-bar',       roles: ['admin'], badge: '2D' },
        ...
        <aside className="sidebar">
            ...
            <nav className="sidebar-nav">
                {visibleNav.map(n => (
                    <button
                        key={n.id}
                        className={`nav-item ${activeTab === n.id ? 'active' : ''} ${n.badge ? 'has-badge' : ''}`}
                        onClick={() => setActiveTab(n.id)}
                    >
                        <i className={n.icon} />
                        <span className="nav-item-label">{n.label}</span>
                        {n.badge && (
                            <span className={`nav-badge nav-badge-${n.id === 'virtual-campus' ? 'purple' : n.id === 'cohort-analytics' ? 'green' : 'new'}`}>
                                {n.badge}
                            </span>
                        )}
                    </button>
